function getGraph_1(month, year) {
	...
	$.ajax({
		url: "../php/ec_antitarnish_main.php",
		type: 'POST',
		...
		success: function(dataset1) {
			...
			var chart = c3.generate({
				...
				regions: [
					{axis: 'y2', start: 0, end: 180, class: 'regionY'}
				],
				
				grid: {
					y: {
						lines: [{value: 0, class: 'gridLimit', text: 'Min : 0 m²', axis: 'y2', position: 'start'}, {value: 180, class: 'gridLimit', text: 'Max : 180 m²', axis: 'y2', position: 'start'}]
					}
				}
...
function getGraph_2(month,year) {
	...
	$.ajax({
		...
		regions: [
			{axis: 'y', start: 35, end: 40, class: 'regionY'}
		],
		grid: {
			y: {
				lines: [{value: 35, class: 'gridLimit', text: 'Min : 35'}, {value: 40, class: 'gridLimit', text: 'Max : 40'}]
			}
		}
    });
	},
