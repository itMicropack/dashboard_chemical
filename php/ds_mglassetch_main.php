...
switch($action) {
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `mglass_etch_amt1`, `mglass_etch_add`, `mglass_etch_amt2`
			
			   FROM `ds_mglass_etch`
		
			   WHERE MONTH(`dateAuto`) = '".$date1."' AND `mglass_etch_amt1` IS NOT NULL AND YEAR(`dateAuto`) = '".$date2."'
	...
	while ($row = mysqli_fetch_array($result_1)) {
		$dataset1['x'][]= $row['dt'];
		$dataset1['data1'][]= $row['mglass_etch_amt1'];
		$dataset1['data2'][]= $row['mglass_etch_add'];
		$dataset1['data3'][]= $row['mglass_etch_amt2'];
	}
