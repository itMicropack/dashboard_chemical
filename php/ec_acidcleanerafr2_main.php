...
switch($action) {
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `afr2_amt1`, `afr2_add`, `afr2_amt2`
			
			   FROM `ec_acidcleaner`
		
			   WHERE MONTH(`dateAuto`) = '".$date1."' AND `afr2_amt1` IS NOT NULL AND YEAR(`dateAuto`) = '".$date2."'
	...
	while ($row = mysqli_fetch_array($result_1)) {
		...
		$dataset1['x'][] = $row['dt'];
		$dataset1['data1'][] = $row['afr2_amt1'];
		$dataset1['data2'][] = $row['afr2_add'];
		$dataset1['data3'][] = $row['afr2_amt2'];
