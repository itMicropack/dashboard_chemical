...
switch($action) {
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `orp_amt1`, `hcl_add`, `h2o2_add`, `orp_amt2`
			
			   FROM `etch_innerlayer`
		
			   WHERE MONTH(`dateAuto`) = '".$date1."' AND `orp_amt1` IS NOT NULL AND YEAR(`dateAuto`) = '".$date2."'
	...
	while ($row = mysqli_fetch_array($result_1)) {
		$dataset1['x'][]= $row['dt'];
		$dataset1['data1'][]= $row['orp_amt1'];
		$dataset1['data2'][]= $row['hcl_add'];
		$dataset1['data3'][]= $row['h2o2_add'];
		$dataset1['data4'][]= $row['orp_amt2'];
	}
