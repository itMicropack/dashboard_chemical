...
switch($action) {
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `cvs_ha`, `cvs_har_add` ,`cvs_val1a`
			
			   FROM `ept_copperbath24_25_rev`
		
			   WHERE MONTH(`dateAuto`) = '".$date1."' AND `cvs_har_add` IS NOT NULL AND YEAR(`dateAuto`) = '".$date2."'
	...
	while ($row = mysqli_fetch_array($result_1)) {
		...
		$dataset1['x'][] = $row['dt'];
		$dataset1['data1'][] = $row['cvs_ha'];
		$dataset1['data2'][] = $row['cvs_har_add'];
		$dataset1['data3'][] = $row['cvs_val1a'];
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `cvs_hl`, `cvs_har_lev` ,`cvs_val3a`
			
			   FROM `ept_copperbath24_25_rev`
		
			   WHERE MONTH(`dateAuto`) = '".$date1."' AND `cvs_har_lev` IS NOT NULL AND YEAR(`dateAuto`) = '".$date2."'
	...
	while ($row = mysqli_fetch_array($result_1)) {
		...
		$dataset1['x'][] = $row['dt'];
		$dataset1['data1'][] = $row['cvs_hl'];
		$dataset1['data2'][] = $row['cvs_har_lev'];
		$dataset1['data3'][] = $row['cvs_val3a'];
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `cvs_hc`, `cvs_har_car` ,`cvs_val2a`
			
			   FROM `ept_copperbath24_25_rev`
		
			   WHERE MONTH(`dateAuto`) = '".$date1."' AND `cvs_har_car` IS NOT NULL AND YEAR(`dateAuto`) = '".$date2."'
	...
	while ($row = mysqli_fetch_array($result_1)) {
		...
		$dataset1['x'][] = $row['dt'];
		$dataset1['data1'][] = $row['cvs_hc'];
		$dataset1['data2'][] = $row['cvs_har_car'];
		$dataset1['data3'][] = $row['cvs_val2a'];
