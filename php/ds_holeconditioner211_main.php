...
switch($action) {
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `chemical_per`, `addition`
			
			   FROM `ds_hole_conditioner_211`
		
			   WHERE MONTH(`dateAuto`) = '".$date1."' AND `chemical_per` IS NOT NULL AND YEAR(`dateAuto`) = '".$date2."'
	...
	while ($row = mysqli_fetch_array($result_1)) {
		$dataset1['x'][]= $row['dt'];
		$dataset1['data1'][]= $row['chemical_per'];
		$dataset1['data2'][]= $row['addition'];
	}
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `bath_strength1`, `add_211`, `bath_strength2`
	...
	while ($row = mysqli_fetch_array($result_1)) {
		...
		$dataset1['data2'][]= $row['add_211'];
		$dataset1['data3'][]= $row['bath_strength2'];
	}
