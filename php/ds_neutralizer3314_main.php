...
switch($action) {
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `alkaline_norm1`, `add_3314`, `alkaline_norm2`
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `bath_strength_amt1`, `add_h2o2`, `bath_strength_amt2`
			
			   FROM `ds_neutralizer_3314`
		
			   WHERE MONTH(`dateAuto`) = '".$date1."' AND `bath_strength_amt1` IS NOT NULL AND YEAR(`dateAuto`) = '".$date2."'
	...
	while ($row = mysqli_fetch_array($result_1)) {
		$dataset1['x'][]= $row['dt'];
		$dataset1['data1'][]= $row['bath_strength_amt1'];
		$dataset1['data2'][]= $row['add_h2o2'];
		$dataset1['data3'][]= $row['bath_strength_amt2'];
	}
