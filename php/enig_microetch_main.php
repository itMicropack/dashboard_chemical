...
switch($action) {
	...
	while ($row = mysqli_fetch_array($result_1)) {
		...
		$dataset1['data1'][]= $row['sa_add'];
		$dataset1['data1'][]= $row['sa_amt2'];
	}
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `etch_rate1`, `securiganth_add`, `etch_rate2`
			
			   FROM `eg_microetch`
		
			   WHERE MONTH(`dateAuto`) = '".$date1."' AND `etch_rate1` IS NOT NULL AND YEAR(`dateAuto`) = '".$date2."'
	...
	while ($row = mysqli_fetch_array($result_1)) {
		...
		$dataset1['data2'][]= $row['securiganth_add'];
