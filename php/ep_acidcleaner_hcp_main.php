...
switch($action) {
	...
	$sql_1 = " SELECT DATE(`dateAuto`) AS dt, `hcp_amt1`, `hcp_add`, `hcp_amt2`
			
			   FROM `ept_acidcleaner_new`
		
			   WHERE MONTH(`dateAuto`) = '".$date1."' AND `hcp_amt1` IS NOT NULL AND YEAR(`dateAuto`) = '".$date2."'
	...
	while ($row = mysqli_fetch_array($result_1)) {
		...
		$dataset1['x'][] = $row['dt'];
		$dataset1['data1'][] = $row['hcp_amt1'];
		$dataset1['data2'][] = $row['hcp_add'];
		$dataset1['data3'][] = $row['hcp_amt2'];
