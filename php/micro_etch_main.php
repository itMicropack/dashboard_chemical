...
switch($action) {
	...
	$sql_1 = " SELECT DATE(`date_auto`) AS dt, `etch_rate1`, `cupra_etch_pt`, `etch_rate2`
	...
	while ($row = mysqli_fetch_array($result_1)) {
		...
		$dataset1['data2'][] = $row['cupra_etch_pt'];
	...
	$sql_1 = " SELECT DATE(`date_auto`) AS dt, `sa_amt`
			
			   FROM `micro_etch`
		
			   WHERE MONTH(`date_auto`) = '".$date1."' AND `sa_amt` IS NOT NULL AND YEAR(`date_auto`) = '".$date2."'
